...
function fillvecs!(model, sentences, vocab; batchsize=128)
    ...
    for i=1:batchsize:length(iwords)    
        ...
        wij = view(iwords,i:j)
        lw = 67; pwind = 81; d = 15; # TODO not put them in hardcoded order.
        cvchar, cvmask = batch4conv(wij, lw, sow, eow)
        push!(wembed, convembed(model, cvchar, cvmask, lw, d, pwind))

        # following 3 lines are changed for convolution case
...
# for the convolutional case the matrix changes
function convertfile2(infile, outfile)
    atr(x)=transpose(Array(x)) # omer stores in row-major Array, we use col-major Array
    d = load(infile); m = d["model"]
    save(outfile, "cembed", m[:cembed], "forw", map(atr,m[:forw]),
         "back",map(atr,m[:back]), "soft",map(atr,m[:soft]), "char", m[:conv],
