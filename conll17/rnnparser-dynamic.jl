...
function main(args="")
    ...
    @add_arg_table s begin
        ...
        ("--dynamic"; action=:store_true; help="train using greedy model moves (as opposed to oracle moves)")
    ...
    for epoch=1:o[:otrain]
        oracletrain(model=pmodel,optim=optim,corpus=corpora[1],vocab=vocab,arctype=o[:arctype],feats=o[:feats],batchsize=o[:batchsize],pdrop=o[:dropout],dynamic=o[:dynamic])
...
function oracletrain(;model=_model, optim=_optim, corpus=_corpus, vocab=_vocab, arctype=ArcHybridR1, feats=FEATS, batchsize=16, maxiter=typemax(Int), pdrop=(0,0), dynamic=false)
    # global grads, optim, sentbatches, sentences
    # srand(1)
    sentbatches = minibatch(corpus,batchsize; maxlen=MAXSENT, minlen=MINSENT, shuf=true)
    nsent = sum(map(length,sentbatches)); nsent0 = length(corpus)
    ...
    @time for sentences in sentbatches
        grads = oraclegrad(model, sentences, vocab, arctype, feats; losses=losses, pdrop=pdrop, dynamic=dynamic)
        update!(model, grads, optim)
        nw = sum(map(length,sentences))
...
function oracleloss(pmodel, sentences, vocab, arctype, feats; losses=nothing, pdrop=(0,0), dynamic=false)
    ...
    while !all(parserdone)
        ...
        for (i,p) in enumerate(parsers)
            ...
            if mcosts[goldmove] == typemax(Cost)
                ...
                if dynamic
                    mmax = 0; smax = -Inf
                    @inbounds for m = 1:size(logp1,1)
                        if mcosts[m] < typemax(Cost) && logp1[m,i] > smax
                            smax = logp1[m,i]; mmax = m
                        end
                    end
                    # mmax = findfirst(x->x<typemax(Cost), mcosts)
                    # mmax = findlast(x->x<typemax(Cost), mcosts)
