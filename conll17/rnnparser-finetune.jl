...
function main(args="")
    ...
    s1 = corpora[1][1]
    fillvecs!(wmodel, [s1], vocab)

    @msg :initmodel
    (pmodel,poptim) = makepmodel(d,o,s1) # corpora[1][1])
    emptyvecs!([s1])
    model = Any[ wmodel, pmodel ]
    optim = initoptim(model,o[:optimization])
    save1(file)=(@msg file; savefile(file, vocab, wmodel, pmodel, poptim, o[:arctype], o[:feats])) #DBG: not saving wmodel right now
    ...
    function report(epoch,beamsize=o[:beamsize])
        ...
        for i=1:length(corpora)
            if o[:report][i] != 0 || (o[:bestfile] != nothing && i==length(corpora))
                las[i] = beamtest(model=model,corpus=corpora[i],vocab=vocab,arctype=o[:arctype],feats=o[:feats],beamsize=beamsize,batchsize=o[:batchsize])
            end
        end
    ...
    for epoch=1:o[:otrain]
        oracletrain(model=model,optim=optim,corpus=corpora[1],vocab=vocab,arctype=o[:arctype],feats=o[:feats],batchsize=o[:batchsize],pdrop=o[:dropout])
    ...
    for epoch=1:o[:btrain]
        beamtrain(model=model,optim=optim,corpus=corpora[1],vocab=vocab,arctype=o[:arctype],feats=o[:feats],beamsize=o[:beamsize],pdrop=o[:dropout],batchsize=1) # larger batchsizes slow down beamtrain considerably
    ...
    if o[:output] != nothing    # TODO: parse all data files?
        ...
        if corpora[1][1].parse == nothing
            @msg :parsing
            beamtest(model=model,corpus=corpora[1],vocab=vocab,arctype=o[:arctype],feats=o[:feats],beamsize=o[:beamsize],batchsize=o[:batchsize])
        end
...
function beamloss(model, sentences, vocab, arctype, feats, beamsize; earlystop=true, steps=nothing, pdrop=(0,0))
    # global parsers,fmatrix,beamends,cscores,pscores,parsers0,beamends0,totalloss,loss,pcosts,pcosts0
    wmodel = model[1]; pmodel = model[2]
    fillvecs!(wmodel, sentences, vocab; pdrop=pdrop)
...
function oracleloss(model, sentences, vocab, arctype, feats; losses=nothing, pdrop=(0,0))
...
function fillvecs!(model, sentences, vocab; batchsize=128, pdrop=(0,0))
    ...
    for i=1:batchsize:length(iwords)    
        ...
        push!(wembed, charlstm(model,cdata,cmask;pdrop=pdrop[1]))    # wembed[C,V] where V: input word vocab, C: charlstm hidden (350)
    end
    ...
    for i=1:batchsize:length(isents)
        ...
        forw,back = wordlstm(model,wdata,wmask,wembed;pdrop=pdrop[2])  # forw/back[T][W,B] where T: longest sentence, B: batch size, W: wordlstm hidden (300)
...
function charlstm(model,data,mask;pdrop=0)
    ...
    if isa(getval(weight),KnetArray); mask = map(KnetArray, mask); end
    @inbounds for t in 1:T
        input = dropout(cembed_t(t), pdrop)
        (hidden,cell) = lstm(weight,bias,hidden,cell,input;mask=mask[t])
    end
    return hidden
end

function wordlstm(model,data,mask,embeddings; pdrop=0) # col-major
