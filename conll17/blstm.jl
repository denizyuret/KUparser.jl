...
function loadmodel()
    global model = Dict()
    m = load("turkish_20kmodel.jld")["model"]
    for k in (:fembed,:bembed); model[k] = KnetArray(m[k]); end
    for k in (:forw,:soft,:back); model[k] = map(KnetArray, m[k]); end
end

function loadvocab()
    global vocab = load("turkish_20kvocab.jld")["vocab"]
end

function loadcorpus()
    global corpus = Vector{Int32}[]
    sentence = Int32[]
    unk = vocab["<unk>"]
    for line in eachline("tr-ud-train.conllu")
#    for line in eachline("okirnap/turkish_dev.conllu")
        line = chomp(line)
        fields = split(line,'\t')
        if line == ""
            if !isempty(sentence)
        ...
        elseif ismatch(r"^\d+$", fields[1])
            push!(sentence, get(vocab, fields[2], unk))
        elseif line[1] == '#'
            # skip
...
# guarantees equal length batch elements, some batches may be smaller
...
# sos/eos padding blstm
function blstm2(model, sequence; result=nothing)
